...
# -c() removes the specified rows or columns from the matrix.
A[-c(1,3),-c(1,3,4)]
dim(A) # Dimensions
...
# read.csv() is generally faster than read.table() for reading CSV files.
...
# na.omit() removes rows with missing values, reducing the number of rows.
names(Auto)
```
...
# Confidence intervals provide the range for the mean response, while prediction intervals account for individual observations and are wider.
